...
# 2026-06-07: 加 90s timeout 避免卡死阻塞主流程
cd "$AI_ROUTER" 2>/dev/null || cd "$(dirname "$AI_ROUTER")"
timeout 90 node ai-router.js grok "$GEO_PROMPT" 2>&1 | tee -a "$LOG_FILE"
GEO_EXIT=$?

if [ $GEO_EXIT -ne 0 ]; then
    log_geo "⚠️ Grok GEO评估超时/失败 (exit=$GEO_EXIT)，跳过 GEO AI 评估"
    log_geo "=========================================="
    exit 0  # 不阻塞主流程
fi
