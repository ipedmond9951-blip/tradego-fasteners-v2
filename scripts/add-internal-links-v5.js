#!/usr/bin/env node
/**
 * Add Internal Links to Articles (v5)
 * Skip matching inside existing </a>...</a> tags
 */
...
function addLinksToText(text) {
  ...
  while (i < text.length) {
    // Skip entire existing anchor tags
    if (text[i] === '<' && text.substring(i, i + 4) === '</a>') {
      result += '</a>';
      i += 5;
      continue;
    }
    ...
    for (const { keyword, url } of PRODUCT_KEYWORDS) {
      ...
      if (substr.toLowerCase() === keyword.toLowerCase()) {
        ...
        if (i > 0 && /[a-zA-Z0-9]/.test(text[i - 1])) continue;
...
console.log('🔗 TradeGo Internal Links Adder v5');
console.log('==================================\n');
