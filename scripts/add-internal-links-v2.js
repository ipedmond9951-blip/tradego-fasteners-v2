#!/usr/bin/env node
/**
 * Add Internal Links to Articles (Corrected v2)
 * Single-pass approach: find all matches first, then replace from end to start
 */
...
function addLinksToText(text) {
  ...
  for (const { keyword, url } of PRODUCT_KEYWORDS) {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const inHref = lastHrefStart > lastHrefEnd;
      
      if (openTags > closeTags || inHref) continue;
  ...
  // Sort by index descending (replace from end to start)
  matches.sort((a, b) => b.index - a.index);
  
  // Apply replacements
  let result = text;
  for (const m of matches) {
    const link = `<a href="${m.url}" class="text-primary-600 hover:text-primary-800 underline underline-offset-2">${result.substring(m.index, m.index + m.length)}</a>`;
    result = result.substring(0, m.index) + link + result.substring(m.index + m.length);
  }
...
console.log('🔗 TradeGo Internal Links Adder v2');
console.log('==================================\n');
