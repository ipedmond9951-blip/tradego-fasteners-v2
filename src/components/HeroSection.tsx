      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10 py-10 md:py-14 lg:py-20">
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
          ...
          <div className="flex-1 w-full max-w-xl">
            ...
            <p className="text-sm md:text-base lg:text-lg text-blue-100/90 mb-5 md:mb-7 leading-relaxed">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2 mb-6 md:mb-8">
              {[t(locale, 'hero.iso'), t(locale, 'hero.global'), t(locale, 'hero.experience')].map((label, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 text-xs md:text-sm font-medium">
                  <svg className="w-4 h-4 md:w-5 md:h-5 text-green-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                  {label}
                </span>
              ))}
            </div>
            <div className="flex flex-wrap gap-3 md:gap-4">
              <a href={`/${locale}#inquiry`} className="inline-flex items-center bg-yellow-400 hover:bg-yellow-300 text-blue-900 px-6 md:px-8 py-2.5 md:py-3 rounded-lg font-bold text-sm md:text-base transition-colors shadow-md">{t(locale, 'hero.cta')}</a>
                ...
                {heroProducts.map((p, i) => (
                  <a key={i} href={`/${locale}#products`} className="group flex items-start gap-3 md:gap-4 p-2 -mx-2 rounded-lg hover:bg-white/[0.08] transition-colors">
                    ...
                    <div className="min-w-0 pt-0.5">
                      ...
                      <p className="text-xs md:text-sm text-blue-200/80 mt-0.5 line-clamp-1">{t(locale, `hero.${p.key}_desc`)}</p>
                    </div>
                  </a>
                ))}
              </div>
