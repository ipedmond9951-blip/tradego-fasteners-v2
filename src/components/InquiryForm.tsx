...
export default function InquiryForm({ locale = 'en' }: InquiryFormProps) {
  ...
  if (submitted) {
    return (
      <section id="inquiry" className="py-12 md:py-20 bg-blue-50">
        <div className="container mx-auto px-4 sm:px-6 text-center">
          <div className="text-4xl md:text-5xl mb-4">✅</div>
          <h2 className="text-lg md:text-2xl font-bold text-blue-900">{t(locale, 'inquiry.success')}</h2>
        </div>
      </section>
    )
  }
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto bg-white rounded-xl md:rounded-2xl shadow-lg p-5 md:p-8 space-y-4 md:space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
            <div>
              ...
              <input name="name" required className="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-1">{t(locale, 'inquiry.email')} *</label>
              <input name="email" type="email" required className="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-1">{t(locale, 'inquiry.company')}</label>
              <input name="company" className="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-1">{t(locale, 'inquiry.country')}</label>
              <input name="country" className="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
          </div>
          <div>
            <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-1">{t(locale, 'inquiry.products')}</label>
            <select name="products" className="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
          ...
          <div>
            ...
            <input name="quantity" className="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="e.g. 5 tons" />
          </div>
          <div>
            <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-1">{t(locale, 'inquiry.message')}</label>
            <textarea name="message" rows={3} className="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
          </div>
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button type="submit" disabled={loading} className="flex-1 bg-blue-900 text-white py-2.5 md:py-3 rounded-lg font-bold hover:bg-blue-800 transition-colors disabled:opacity-50 text-sm md:text-base">
